...
swing_state_polls_dem <- swing_state_polls |>
  ...
  select(year, state, support_DEM_1,  support_REP_1, polling_trend_5_1, polling_trend_10_6, average_polling, current_week)
...
for (state_name in swing_states) {
 ...
 # print(state_name)
  ...
  mod_2 <- lm(D_pv2p ~ state + polling_trend_5_1 + polling_trend_10_6 + current_week + support_DEM_1 , data = train_data)
...
# Display the table using kable for nicely formatted output
in_rmse <- rmse_df %>%
  kable("html", caption = "In-Sample RMSE for 2020 by State", digits = 4) %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  column_spec(1, width = "8em", extra_css = "text-align: center;") %>%
...
# Display the table using kable for nicely formatted output
out_rmse <- rmse_df %>%
  kable("html", caption = "Out-of-Sample RMSE for 2020 by State", digits = 4) %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  column_spec(1, width = "8em", extra_css = "text-align: center;") %>%
  column_spec(2, width = "6em", extra_css = "text-align: center;")
...
map_plot <- plot_ly(
  ...
  zmid = 0,     
  zmax = 7.5,    
  colorbar = list(title = "Dem. Voting Margin (%)")
) %>%
  layout(
    title = list(
...
for (state_name in swing_states) {
  ...
  mod_2 <- lm(D_pv2p ~ state + polling_trend_5_1 + polling_trend_10_6 + current_week + support_DEM_1, data = state_data)
...
sim_plot <- ggplot(expanded_data, aes(x = electoral_votes, fill = color)) +
  geom_dotplot(binwidth = 1.5, dotsize = 0.5, stackdir = "up", stroke = 0) +
  scale_x_continuous(breaks = seq(0, 538, by = 10), limits = c(220, 310)) +
  scale_fill_identity() + 
  labs(
    title = "Simulated Electoral College Outcomes",
    subtitle = "Each dot represents one simulation result",
