...
state_pv = read.csv("state_2pv_1948_2020.csv")
...
national <- read.csv("data/nat_pv_1860_2020.csv")

national_mutate = national |>
  mutate(dem_tpv = (npv_democrat / (npv_democrat + npv_republican)) * 100,
         rep_tpv = (npv_republican / (npv_democrat + npv_republican)) * 100) |>
...
state_select |>
  ...
  group_by(state) |>
  summarize(mean = mean(two_party_vote_share)) |>
...
combined_data = state_select |>
  full_join(national_mutate_filter_long, by = c("year","party"))
...
combined_data_wide = state_select |>
  pivot_wider(names_from = c("party","state"), values_from = "two_party_vote_share")
...
model = lm(data = combined_data_wide, dem_tpv ~ Democrat_Florida)
summary(model)
...
```{r}
m_model = lm(data = combined_data_wide, dem_tpv ~ Democrat_Florida + `Democrat_New York`)
...
predict(m_model, newdata = newdata, type = "response")
